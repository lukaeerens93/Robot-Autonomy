...
def main(robot, planning_env, planner,args):
    ...
    # plan, visited= planner.Plan(start_config, goal_config)
    dist = planning_env.SetGoalParameters(goal_config)
    start = time.time()
    plan, plan_node= planner.Plan(start_config, goal_config)
    stop = time.time()
    # plan = plan[:-1]
    with open('robot_autonomy.txt', 'a') as f:  # Python 3: open(..., 'wb')
        f.write("planning_env:{}\nplanner:{}\n resolution:{}\nplan:{}\n\
        time:{}\nnodes number:{}\n".format(planning_env,\
         planner,args.resolution, plan, stop - start, len(plan_node)))
...
if __name__ == "__main__":
    ...
    main(robot, planning_env, planner,args)
